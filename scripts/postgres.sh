#!/bin/bash -e
#
# postgres.sh - Change Database to PostgreSQL for one or all Joomla container.
#   scripts/postgres.sh
#   scripts/postgres.sh 51
#
...
for version in "${versions[@]}"; do
  ...
  log "jbt_${version} – Create cypress.config.${extension} for PostgreSQL"
  # adopt e.g.:
  ...
  log "jbt_${version} – PostgreSQL based Joomla is installed"
  echo ""
