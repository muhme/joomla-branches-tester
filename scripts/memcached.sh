...
for instance in "${instancesToChange[@]}"; do
  ...
  if [ "${todo}" = "on" ]; then
    if ((instance == 310 || instance < 40)); then
      warning "jbt-${instance} – Memcached is only supported for Joomla >= 4.0, skipped over"
    elif docker exec "jbt-${instance}" bash -c "grep -q \"cache_handler = 'memcached'\" configuration.php"; then
      log "jbt-${instance} – Memcached cache is already enabled"
    else
      log "jbt-${instance} – Enabling Memcached cache handler (host '${JBT_MEMCACHED_HOST}', port ${JBT_MEMCACHED_PORT})"
      
      docker exec "jbt-${instance}" bash -c "sed \
        -e \"s|\(public .caching =\).*|\1 1;|\" \
        -e \"s|\(public .cache_handler =\).*|\1 'memcached';|\" \
        -e \"s|\(public .memcached_server_host =\).*|\1 '${JBT_MEMCACHED_HOST}';|\" \
        -e \"s|\(public .memcached_server_port =\).*|\1 ${JBT_MEMCACHED_PORT};|\" \
        -e \"s|\(public .session_memcached_server_host =\).*|\1 '${JBT_MEMCACHED_HOST}';|\" \
        -e \"s|\(public .session_memcached_server_port =\).*|\1 ${JBT_MEMCACHED_PORT};|\" \
