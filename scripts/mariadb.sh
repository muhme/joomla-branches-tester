#!/bin/bash -e
#
# mariadb.sh - Change Database to MariaDB for one or all Joomla container.
#   scripts/mariadb.sh
#   scripts/mariadb.sh 51
#
...
for version in "${versions[@]}"; do
  ...
  log "jbt_${version} – Create cypress.config.${extension} for MariaDB"
  # adopt e.g.:
  ...
  log "jbt_${version} – MariaDB based Joomla is installed"
  echo ""
